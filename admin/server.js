...
app.use(async (req, res, next) => {
    ...
    let item = req.$TOKENDATA.find(item => {
        return item.token === authorization && item.sign === sign && item.time === time;
    });
    let temp = md5(`${authorization}@${time}@zhufeng`);
    if (item || temp !== sign) {
        responsePublic(res, false, {
    ...
    req.$TOKENDATA.push({ token: authorization, sign, time });
...
app.post('/user_update', async (req, res) => {
    try {
        ...
        let file = (files.file && files.file[0]) || {},
            username = (fields.name && fields.name[0]) || "",
